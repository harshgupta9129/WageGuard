...
import { Phone, Lock, Sun, Moon, ShieldAlert, ArrowRight, ShieldCheck, UserCheck, Loader2 } from "lucide-react";
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setIsSuccess(true);
      // Wait 1.8s for smooth UI transition
      setTimeout(() => {
        login(res.data);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 font-sans px-4 relative overflow-hidden bg-grid-pattern transition-colors duration-500">
      ...
      {isSuccess ? (
        /* Smooth Verification Transition Card */
        <div className="relative w-full max-w-md glass-panel border p-8 rounded-3xl shadow-xl flex flex-col items-center justify-center text-center animate-scale-in py-12">
          ...
          <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2 font-display bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-indigo-900 dark:from-white dark:to-indigo-200">
            Sign In Successful
          </h3>
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mb-8 max-w-xs">
            Securing your connection and redirecting to your Wage Guard portal...
          </p>
          <div className="flex items-center gap-2.5 text-indigo-600 dark:text-indigo-400 font-bold text-xs uppercase tracking-widest">
        ...
        <div className="relative w-full max-w-md glass-panel border p-8 rounded-3xl shadow-xl hover:border-indigo-500/20 dark:hover:border-indigo-500/20 glow-indigo animate-scale-in">
          ...
          <div className="mt-6 border-t border-slate-150 dark:border-slate-800/80 pt-5">
            <p className="text-xs font-semibold text-slate-400 dark:text-slate-550 text-center uppercase tracking-wider mb-3">
              Demo Credentials
            </p>
